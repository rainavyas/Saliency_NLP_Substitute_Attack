...
if __name__ == '__main__':
    ...
    original_negs = get_pca_principal_components(eigenvectors, correction_mean, embeddings, num_comps, 0)

    embeddings = batched_get_layer_embedding(original_list_pos, handler, tokenizer, device)
    original_poss = get_pca_principal_components(eigenvectors, correction_mean, embeddings, num_comps, 0)

    embeddings = batched_get_layer_embedding(attack_list_neg, handler, tokenizer, device)
    attack_negs = get_pca_principal_components(eigenvectors, correction_mean, embeddings, num_comps, 0)
